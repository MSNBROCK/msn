...
Next eligible withdrawal date: November 29, 2026..</p><button class="btn secondary" onclick="navigate('Platform Updates')">View updates</button></div></div>${notice()}`,
 ...
 Notifications:()=>`<div class="two"><div class="card"><div class="section-head"><h2>Leader notification center</h2><button class="btn" onclick="openNotification()">Write notification</button></div><p class="muted">Send a platform announcement to all 11 paper client profiles or select recipients.</p>${adminNotifications.length?adminNotifications.slice(-8).reverse().map(n=>`<div class="kpi" style="margin:9px 0"><small>${n.date} · ${n.recipients}</small><b>${n.title}</b><p class="muted">${n.message}</p></div>`).join(""):`<div class="empty">No leader notifications yet.</div>`}</div><div class="card"><h2>System notifications</h2>${["Advanced portfolio analytics are available.","₹1,500 pending paper-payment record.","Market-data API connection is ready for configuration."].map((x,i)=>`<div class="kpi" style="margin:9px 0"><small>${i===0?"New feature":"Account update"}</small><b>${x}</b></div>`).join("")}</div></div>`,
...
function openTransfer(preselect=""){modal("Transfer paper record",`<p class="muted">Records a simulated transfer between client profiles.  real funds move.</p><div class="field"><label>FROM CLIENT</label><select id="trFrom" class="search">${clientOptions(preselect||current.id)}</select></div><div class="field"><label>TO CLIENT</label><select id="trTo" class="search">${clientOptions()}</select></div><div class="field"><label>AMOUNT</label><input id="trAmount" type="number" min="0" placeholder="Enter amount"></div><div class="field"><label>NOTE</label><input id="trNote" class="search" placeholder="Transfer note"></div><button class="btn" onclick="submitTransfer()">Submit paper transfer</button>`)}
